...
for i in "${!privateKeys[@]}"; do
    ...
    pk=${privateKeys[$i]}
    proxy=${proxies[$i]}
    until env http_proxy="http://$proxy" https_proxy="http://$proxy" no_proxy="localhost,127.0.0.1" \
        ...
        PRIVATE_KEY=$pk \
        ./light-node >> "executor_${i}.log" 2>&1
    do 
        echo "Retrying... Proxy: ${proxies[$i]}" >> "executor_${i}.log"
        sleep 1
    done &
    sleep 1
done
